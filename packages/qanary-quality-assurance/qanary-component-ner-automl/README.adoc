= Qanary Tests for compontent "qanary-component-ner-automl"
:toc:
...
.. *1_was-the-expected-instance-recognized.sparql:* Validate if the pre-defined values for test-case-specific replacements/placeholders are available in the graph as annotation of type `AnnotationOfInstance`.
...
[,shell]
----
python evaluate-qanary-system.py --directory=qanary-component-ner-automl
----
