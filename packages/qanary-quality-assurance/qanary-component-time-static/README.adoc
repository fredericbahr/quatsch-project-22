...
This directory contains the tests for `qanary-component-time` where static time values get stored in triple store, e.g. 23.01.2023.
...
.. *1_was-the-expected-instance-recognized.sparql:* Validate if the pre-defined values for test-case-specific replacements/placeholders are available in the graph as annotation of type `AnnotationOfTime`.
...
[,shell]
----
python evaluate-qanary-system.py --directory=qanary-component-time-static
----
