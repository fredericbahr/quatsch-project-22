...
def request_qanary_endpoint_for_question(logger, conf_qanary, question):
    ...
    logger.debug("additionalTriples" + additionalTriples)
...
def export_accumulated_metrics_to_txt(logger, test_results, filename_prefix):
    unique_questions = []
    result_counts = []

    for result in test_results[0].get("results"):
        result_counts.append({list(result.keys())[0]: 0})

    for test_result in test_results:
        if test_result.get("question") not in unique_questions:
            unique_questions.append(test_result.get("question"))

        for index, result in enumerate(test_result.get("results")):
            key = list(result.keys())[0]
            if result.get(key) == True:
                previous_value = result_counts[index].get(key)
                result_counts[index].update({key: previous_value + 1})

    any_instance_result = result_counts[0].get(list(result_counts[0].keys())[0])
    any_instance_percentage = (any_instance_result / len(test_results)) * 100
    expected_instance_result = result_counts[1].get(list(result_counts[1].keys())[0])
    expected_instance_percentage = (expected_instance_result / len(test_results)) * 100

    txt_filecontent = f'''============================================================
Total Questions: {len(test_results)}
Unique Questions: {len(unique_questions)}

Any Instance Identified: {round(any_instance_percentage, 2)}% ( {any_instance_result}/{len(test_results)} )
Expected Instance Recognized: {round(expected_instance_percentage, 2)}% ( {expected_instance_result}/{len(test_results)} )
============================================================
    '''

    txt_filename = filename_prefix + ".txt"
    # writing accumulated metrics to txt file
    with open(txt_filename, "w") as outfile:
        outfile.write(txt_filecontent)
        print("TXT file written to '%s'." % (txt_filename,))
...
def main(logger, sheet_name, configuration_directory, outdir, filename_prefix):
    ...
    export_accumulated_metrics_to_txt(logger, test_results, filename_prefix)
