...
Custom training data for this Qanary NER component can be generated. Lubw domain instances for stations, measurands, calculations and representations will be combined with a set of training questions.
...
|===
|Command |Description
|`npm run generate:csv` |generates NER training and testing data as CSV files for immediate training on container startup
|`npm run generate:json` |generates NER training and testing data as JSON files for retraining via `/retrain` route
|`npm run generate:yml` |generates NLU training data as YML file for Rasa NLU slot filling
|===
